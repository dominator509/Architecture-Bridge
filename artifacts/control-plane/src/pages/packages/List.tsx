    ...
    <div className="p-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-card rounded-lg border border-dashed">
          ...
          <h3 className="text-lg font-medium">No packages</h3>
        </div>
      ) : (
              ...
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-6 py-4 font-medium">
                    <Link href={`/tenants/${tenantId}/packages/${p.id}`} className="text-primary hover:underline" data-testid={`link-package-${p.id}`}>
                      {p.name}
                    </Link>
